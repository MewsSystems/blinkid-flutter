...
class MicroblinkScanner {
  ...
  static Future<List<RecognizerResult>> scanWithCamera(
      RecognizerCollection collection, OverlaySettings overlaySettings, String license) async {
    var jsonResults = jsonDecode(await _channel.invokeMethod(METHOD_SCAN_WITH_CAMERA, {
  ...
  static Future<List<RecognizerResult>> scanWithDirectApi(
      RecognizerCollection collection, String frontImage, String? backImage, String license) async {
    var jsonResults = jsonDecode(await _channel.invokeMethod(METHOD_SCAN_WITH_DIRECT_API, {
      ARG_RECOGNIZER_COLLECTION: jsonDecode(jsonEncode(collection)),
