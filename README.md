...
> **Note:** The `sample` directory is generated by the script and should not be edited directly. Sample source files live in [`sample_files/`](sample_files/). To test a local plugin build before release, set `IS_LOCAL_BUILD=true` at the top of `initBlinkIdFlutterSample.sh` (this is the default in the repository).
...
Use Kotlin **2.1.20** or newer in `android/settings.gradle.kts` (match the version declared for `org.jetbrains.kotlin.android`).
